    ...
    <div>
      <div class="tag">
        <a-badge class="info-box"
          ...
          <div>
            <a-avatar class="avatar" :size="60" :src="item.logo" @click="routerLabelToPost(item.id)"/>
          </div>
          <div class="title" @click="routerLabelToPost(item.id)">{{ item.labelName }}</div>
          <div class="meta-post">{{ item.postUseCount + ' ' + $t('common.post') }}</div>
